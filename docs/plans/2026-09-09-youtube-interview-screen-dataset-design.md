...
自动字幕只作为文本参考，绝不标成项目 ASR 或人工转写。若没有字幕，默认跳过该视频；只有显式指定允许无字幕时才保留，并标记 `caption_status=missing`。
...
- 不跨越明显长空白，也不生成负时间、零时长或越过父视频结尾的切片；
...
父视频和短片都保存：schema 版本、profile、YouTube 视频/频道 ID、标题、公开页面 URL、发布时间、抓取时间、时长、语言、字幕来源、说话人数及其核验状态、许可/权利备注、yt-dlp 版本、媒体流信息和 SHA-256。
